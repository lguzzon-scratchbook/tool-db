...
const index_3 = __importDefault(
  require('../packages/websocket-network/dist/index')
)
...
const checkIfOk = (id) => {
  if (!connected.includes(id)) {
    ...
    if (connected.length === 4) {
      console.log(
        '\n          test-node-a: '
          .concat(nodeA.network.getClientAddress(), '\n          test-node-b: ')
          .concat(nodeB.network.getClientAddress(), '\n          test-alice: ')
          .concat(Alice.network.getClientAddress(), '\n          test-bob: ')
          .concat(Bob.network.getClientAddress(), '\n          test-chris: ')
          .concat(Chris.network.getClientAddress(), '\n        ')
      )
    }
  }
}
